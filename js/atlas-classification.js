(function initializeAtlasClassification(root, factory) {
  ...
  function classifyGrowthType(countyName) {
    ...
    if (!normalized) return '\u{1F33D} Rural / Local Growth';

    const metroAndUniversity = new Set([
      'POLK', 'DALLAS', 'WARREN', 'STORY', 'LINN', 'JOHNSON', 'SCOTT',
      'BLACK HAWK', 'DUBUQUE'
    ]);
    const i80Corridor = new Set([
      'POTTAWATTAMIE', 'CASS', 'ADAIR', 'MADISON', 'DALLAS', 'POLK',
      'JASPER', 'POWESHIEK', 'IOWA', 'JOHNSON', 'CEDAR', 'SCOTT'
    ]);
    const mississippiRiver = new Set([
      'ALLAMAKEE', 'CLAYTON', 'DUBUQUE', 'JACKSON', 'CLINTON', 'SCOTT',
      'MUSCATINE', 'LOUISA', 'DES MOINES', 'LEE'
    ]);

    if (metroAndUniversity.has(normalized)) return '\u{1F3D9}\uFE0F Metro / University Growth';
    if (i80Corridor.has(normalized)) return '\u{1F6E3}\uFE0F I-80 Corridor';
    if (mississippiRiver.has(normalized)) return '\u{1F30A} Mississippi River Corridor';
    return '\u{1F33D} Rural / Local Growth';
  }
